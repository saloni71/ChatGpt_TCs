...
let options = require('../support/report_options').options;
...
const PreApiCall = require('../script/utils/PreApiCall')
...
exports.config = {
          ...
          /*  prefs:
            {
                download: {
                    ...
                    'default_directory': path.join(process.cwd(), REPORT_PATH),
                },
    ...
    reporters: [
        ...
        ['cucumberjs-json', {
            jsonFolder: REPORT_PATH + 'cucumber/',
            language: 'en',
            disableHooks: true
        }]
    ],
    ...
    onPrepare: async function () {
        console.log("=================== Pre API Check START ===============================")
        if (!existsSync(REPORT_PATH)) {
            mkdirsSync(REPORT_PATH);
        }
        console.log("=================== Pre API Check END ===============================")
    },
    ...
    */
    before: function () {
        const { Given, When, Then, AfterAll, Before, After, BeforeAll } = require("@wdio/cucumber-framework");
...
function getArgs() {
    if (process.argv.includes('--headless')) {
        ...
        return ['--start-maximized', '--log-level=3', '--disable-infobars', '--ignore-certificate-errors', '--incognito'];
    }
}
